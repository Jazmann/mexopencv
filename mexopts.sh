...
#----------------------------------------------------------------------------
			...
			XCODEVERSION=`xcodebuild -version| grep "Xcode" | sed 's/.*Xcode.\([0-9]\.[0-9]\).*/\1/'`
            XCODEMAJOR=`echo $XCODEVERSION | sed 's/\([0-9]*\)\.[0-9]*.*/\1/'`
            XCODEMINOR=`echo $XCODEVERSION | sed 's/[0-9]*\.\([0-9]*\).*/\1/'`
            if [ "$XCODEMAJOR" != "" ]; then
				if [ "$XCODEMINOR" != "" ]; then
					USELLVM=1
					APPSDKROOT=1;
					if [ $XCODEMAJOR -ge 4 ]; then
						if [ $XCODEMINOR -eq 1 ]; then
							USELLVM=0;
							APPSDKROOT=0;
						elif [ $XCODEMINOR -eq 2 ]; then
							USELLVM=1;
							APPSDKROOT=0;
						fi
					else
					...
					if [ $USELLVM -eq 0 ]; then
						CC='gcc'
						CXX='g++'
					fi
					#if [ $APPSDKROOT -eq 0 ]; then
						#SDKROOT='/Developer/SDKs/MacOSX10.9.sdk'
					#fi
				fi
			fi
...
#
            ...
            # C++keyLanguage: C++
            # C++keyVersion: 
            CXXFLAGS=" -std=c++11 -stdlib=libc++ -fno-common -no-cpp-precomp -fexceptions -arch $ARCHS -isysroot $SDKROOT -mmacosx-version-min=$MACOSX_DEPLOYMENT_TARGET"
            CXXLIBS="$MLIBS -lc++"
